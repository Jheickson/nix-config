...
-- Inline image previews use the Kitty graphics protocol, supported by Ghostty.
-- image.nvim discovers Markdown image links and renders them next to the source.
...
if image_ok then
	image.setup({
		...
		-- Clear images when floating windows overlap them, then restore them
		-- after the overlay closes. Kitty images otherwise sit above Neovim UI.
...
local function image_path_under_cursor()
	...
	if buffer_path:match("%.[Pp][Nn][Gg]$")
		or buffer_path:match("%.[Jj][Pp][Ee]?[Gg]$")
		or buffer_path:match("%.[Gg][Ii][Ff]$")
		or buffer_path:match("%.[Ww][Ee][Bb][Pp]$")
		or buffer_path:match("%.[Aa][Vv][Ii][Ff]$") then
		return buffer_path
	end
...
local function preview_image_under_cursor(notify)
	...
	local options = {
		...
		x = 0,
		y = vim.api.nvim_win_get_cursor(0)[1] - 1,
		inline = true,
...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function(event)
		local map = function(keys, command, desc)
			vim.keymap.set("n", keys, "<cmd>" .. command .. "<CR>", { buffer = event.buf, desc = "LaTeX: " .. desc })
		end
...
-- Minimap is disabled by default; toggle it with <leader>m.
vim.keymap.set("n", "<leader>tm", MiniMap.toggle, { desc = "Toggle minimap" })
...
vim.keymap.set("n", "<leader>bf", MiniPick.builtin.buffers, { desc = "Open buffers" })
