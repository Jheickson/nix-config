...
let
  ...
  # Use the generated output only when wallpaper processing is enabled;
  # otherwise wallpaperSource is the applied wallpaper directly.
  wallpaperPath =
    if stylixConfig.processedWallpaper then
      ...
      toString (
        builtins.path {
          path = stylixConfig.wallpaperSource;
          name = "wallpaper";
        }
      );
